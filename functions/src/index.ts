 ...
 */
export const checkAdminExists = onCall({ region: 'us-central1', cors: true }, async (request: CallableRequest) => {
    try {
 ...
 */
export const updateUserRole = onCall({ region: 'us-central1', cors: true }, async (request: CallableRequest) => {
    if (!request.auth || !await isAdmin(request.auth?.uid)) {
 ...
 */
export const setupAdminUser = onCall({ region: 'us-central1', cors: true }, async (request: CallableRequest) => {
 ...
 */
export const createAdminUser = onCall({ region: 'us-central1', cors: true }, async (request: CallableRequest) => {
    if (!request.auth || !await isAdmin(request.auth?.uid)) {
